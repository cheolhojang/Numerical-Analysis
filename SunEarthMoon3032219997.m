...
set(gcf,'name','Sun, Earth, Mercury and Moon Orbits')
for i = 1:length(t)/q
    subplot(2,2,1)
    plot3(x(1:i*q,1),x(1:i*q,2),x(1:i*q,3),'g',x(1:i*q,7),x(1:i*q,8),x(1:i*q,9),'r',x(1:i*q,13),x(1:i*q,14),x(1:i*q,15),'b',x(1:i*q,19),x(1:i*q,20),x(1:i*q,21),'yellow')
    ...
    subplot(2,2,3)
    plot(x(1:i*q,1),x(1:i*q,2),'g',x(1:i*q,7),x(1:i*q,8),'r',x(1:i*q,19),x(1:i*q,20),'yellow')
    axis(1.5*[min(x(:,1)) max(x(:,1)) min(x(:,2)) max(x(:,2))])
    xlabel('Universal X Coordinate (m)')
    ylabel('Universal Y Coordinate (m)')
    title('Relative Earth, Mecury Orbit About Sun')
    hold on
    plot(x(i*q,1),x(i*q,2),'g-o',x(i*q,7),x(i*q,8),'r-o',x(i*q,19),x(i*q,20),'yellow-o')
...
d_merc_sun = sqrt((u( 19,1)-u(7,1))^2+(u( 20,1)-u(8,1))^2+(u( 21,1)-u(10,1))^2);
d_merc_sat = sqrt((u( 19,1)-u(13,1))^2+(u( 20,1)-u(14,1))^2+(u( 21,1)-u(15,1))^2);
d_earth_merc = sqrt((u( 19,1)-u(1,1))^2+(u( 20,1)-u(2,1))^2+(u( 21,1)-u(3,1))^2);
...
udot( 2,1) = u(5,1);
udot( 3,1) = u(6,1);
udot( 4,1) = m_sun*G*(u(7,1)-u(1,1))/d_earth_sun^3 + m_sat*G*(u(13,1)-u(1,1))/d_earth_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_earth_merc^3;
udot( 5,1) = m_sun*G*(u(8,1)-u(2,1))/d_earth_sun^3 + m_sat*G*(u(14,1)-u(2,1))/d_earth_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_earth_merc^3;
udot( 6,1) = m_sun*G*(u(9,1)-u(3,1))/d_earth_sun^3 + m_sat*G*(u(15,1)-u(3,1))/d_earth_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_earth_merc^3;
% Sun Motion
udot( 7,1) = u(10,1);
udot( 8,1) = u(11,1);
udot( 9,1) = u(12,1);
udot(10,1) = m_earth*G*(u(1,1)-u(7,1))/d_earth_sun^3 + m_sat*G*(u(13,1)-u(7,1))/d_sun_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_merc_sun^3;
udot(11,1) = m_earth*G*(u(2,1)-u(8,1))/d_earth_sun^3 + m_sat*G*(u(14,1)-u(8,1))/d_sun_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_merc_sun^3;
udot(12,1) = m_earth*G*(u(3,1)-u(9,1))/d_earth_sun^3 + m_sat*G*(u(15,1)-u(9,1))/d_sun_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_merc_sun^3;
% Satellite Motion
udot(13,1) = u(16,1);
udot(14,1) = u(17,1);
udot(15,1) = u(18,1);
udot(16,1) = m_earth*G*(u(1,1)-u(13,1))/d_earth_sat^3 + m_sun*G*(u(7,1)-u(13,1))/d_sun_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_merc_sat^3;
udot(17,1) = m_earth*G*(u(2,1)-u(14,1))/d_earth_sat^3 + m_sun*G*(u(8,1)-u(14,1))/d_sun_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_merc_sat^3;
udot(18,1) = m_earth*G*(u(3,1)-u(15,1))/d_earth_sat^3 + m_sun*G*(u(9,1)-u(15,1))/d_sun_sat^3 + m_mercury*G*(u(13,1)-u(19,1))/d_merc_sat^3;
% Mercury Motion
udot(19,1) = u(22,1);
udot(20,1) = u(23,1);
udot(21,1) = u(24,1);
udot(22,1) = m_earth*G*(u(1,1)-u(19,1))/d_earth_merc^3 + m_sun*G*(u(7,1)-u(19,1))/d_merc_sun^3 + m_sat*G*(u(13,1)-u(19,1))/d_merc_sat^3;
udot(23,1) = m_earth*G*(u(2,1)-u(20,1))/d_earth_merc^3 + m_sun*G*(u(8,1)-u(20,1))/d_merc_sun^3 + m_sat*G*(u(14,1)-u(20,1))/d_merc_sat^3;
udot(24,1) = m_earth*G*(u(3,1)-u(21,1))/d_earth_merc^3 + m_sun*G*(u(9,1)-u(21,1))/d_merc_sun^3 + m_sat*G*(u(15,1)-u(21,1))/d_merc_sat^3;
end